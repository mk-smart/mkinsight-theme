...
<?php if(!@$_GET['data'] && !@$_GET['build-chart'] && !@$_GET['chart'] && !@$_GET['chart-editor'] ) : ?>
  <section>
    ...
    <select class="selectpicker show-tick" style="color: #000;" id="mki_select_data">
      ...
      <?php if($data_file->post_parent != $last_parent):?>
        ...
        <optgroup
          ...
          data-post-parent="<?php print $data_file->post_parent; $last_parent = $data_file->post_parent; ?>">
      <?php else: ?>
      <?php endif; ?>
      <option
    ...
    <button class="btn btn-default" style="color: #000;" type="button" onClick="mki_goto_table()">Start</button>
  </section>
<?php endif; ?>
<?php /* END SELECT FILE */ ?>
 ...
 <section>
   ...
   <section>
     <form method="GET" class="form-horizontal">
       ...
       Indicate whether the data starts at a specific row. Tip: the first row should include columns names (see below preview).</p></div>
     <div class="form-group">
       <label class="control-label col-sm-2" for="cr">Data starts at row</label>
       <div class="col-sm-10">
         <input size="5" class="" name="cr" value="<?php print (is_numeric(@$_GET['cr']))?$_GET['cr']:0; ?>" />
       </div>
     </div>
     <div class="form-group"><p class="col-sm-12">Select one column containing the entities and one containing the values.</p></div>
       <div class="form-group">
        <label class="control-label col-sm-2" for="cv">Entities from column:</label>
        <div class="col-sm-10">
        <select class="selectpicker show-tick " name="ce" onChange="$('#mki_use_entity_example').html($(this).find(':selected').data('examples'));">
           <?php foreach($columns as $cix => $col): ?>
               <option <?php print ($cix == @$_GET['ce'])?'SELECTED':''; ?>
                value="<?php print $cix; ?>" data-subtext="<?php print $examples[$cix][0]; ?>" data-examples="E.g.: <?php print implode(', ', $examples[$cix]); ?>"><?php print $col; ?></option>
          <?php endforeach; ?>
        </select>
        </div>
        <div class="col-sm-offset-2 col-sm-10"><small id="mki_use_entity_example">&nbsp;</small></div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="cv">Values from column:</label>
        <div class="col-sm-10">
          <select class="selectpicker show-tick " name="cv" onChange="$('#mki_use_value_example').html($(this).find(':selected').data('examples'));">
             <?php foreach($columns as $cix => $col):?>
        ...
        <div class="col-sm-offset-2 col-sm-10"><small id="mki_use_value_example">&nbsp;</small></div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="vt">Value Type</label>
        <div class="col-sm-10">
          <select class="selectpicker show-tick " name="vt">
             <option <?php print ('number' == @$_GET['vt'])?'SELECTED':''; ?> value="number">Number</option>
             <option <?php print ('string' == @$_GET['vt'])?'SELECTED':''; ?> value="string">String</option>
          </select>
        </div>
      </div>
      <!-- div class="form-group">
      ...
      <div class="form-group">
          ...
          <button class="btn btn-default" type="submit" name="data" value="<?php print $_GET['data'];?>">Preview data</button>
          <button name="chart-editor" value="<?php print $post->ID; ?>" class="btn btn-default" type="submit">Edit chart</button>
        </div>
      </div>
    </form>
  <h3>Preview</h3>
   <div style="color: #000; border: 1px solid #AAA; padding:0px" id="mki_data_div">Loading table ...</div>
 </section>
<?php endif; ?>
<?php /* END DATA SPEC */ ?>
...
<?php
 ...
 <canvas style="display:none" id="hidden_canvas" style="height: 400px; width: 600px;"></canvas>
 ...
 <!-- <a id="downloadJPEG" download="chart.jpeg" class="btn btn-default">JPEG</a> -->
   ...
   <a id="attachSvgToPost" onClick="saveAsPostAttachment('svg')" class="btn btn-default">Attach SVG to page</a>
   <a id="attachPngToPost" onClick="saveAsPostAttachment('png')" class="btn btn-default">Attach PNG to page</a>
